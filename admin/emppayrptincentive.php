...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"emppayrptincentive.php?loginid=$loginid&submitsw=1\" method=\"post\" name=\"emppayrptbasic\">";
	...
	// if(($datefrom <= $dateto) && (($datefrom != '') && ($dateto != ''))) {
		...
		echo "<tr><th colspan=\"2\" align=\"left\">Employees Payroll - Incentive&nbsp;<a href=\"#\" id=\"exportToExcel\"><img src=\"./images/sheet.gif\"></a></th></tr>";
    ...
    echo "<tr><td colspan=\"2\">";
		...
		if($result14 != "") {
			...
			if($result15 != "") {
				...
				$result16 = mysql_query("SELECT otherincometaxable FROM tblemppayroll WHERE employeeid=\"$employeeid14\" AND cut_start=\"$cut_start15\" AND cut_end=\"$cut_end15\"", $dbh);
				if($result16 != "") {
					while($myrow16 = mysql_fetch_row($result16)) {
					$found16 = 1;
					$otherincometaxable16 = $myrow16[0];
					}
				}

				$otherincometaxabletot = $otherincometaxabletot + $otherincometaxable16;

				if($found16 == 1) {
					echo "<td align=\"right\">".number_format($otherincometaxable16, 2)."</td>";
				} else {
					echo "<td></td>";
				}
				$otherincometaxable16=0;
				}
			}

			echo "<th>".number_format($otherincometaxabletot, 2)."</th></tr>";
			$empidtmp = $employeeid14;
			$otherincometaxabletot=0;
			}
		}
