...
//	  $employeeid = $myrow0[0];
	...
	$result = mysql_query("INSERT INTO tblprojcdassign SET timestamp=\"$now\", loginid=$loginid, datecreated=\"$datenow\", projcode=\"$proj_code1\", projname=\"$proj_sname\", empid=\"$employeeid\", projassignid=$projassignid, projectid=$projectid, duration=$duration, durationprop=\"$durationprop\"", $dbh) or die ("Couldn't execute query.".mysql_error());

// create log
    $result16 = mysql_query("SELECT adminuid FROM tbladminlogin WHERE adminloginid=$loginid", $dbh);
    while($myrow16 = mysql_fetch_row($result16))
    { $adminuid=$myrow16[0]; }
    $adminlogdetails = "$loginid:$adminuid - Add project:$projectid-$proj_code1 in project assignment id:$projassignid for $employeeid-$name_last, $name_first $name_middle[0], duration:$duration $durationprop";
