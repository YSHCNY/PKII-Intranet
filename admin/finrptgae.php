    ...
    echo "<tr><td colspan=\"2\">";
		...
		if($result11 != "") {
			...
			*/
				...
				if(strpos($datefrom,  '-07-01') !== false ){
					$beginningBalance = 0;
				}
				else{
        ...
        $res16query="SELECT acctspayableid, acctspayablenumber, debitamt, creditamt FROM tblfinacctspayable WHERE (date>=\"$beginningFrom\" AND date<=\"$beginningTo\") AND (glcode>=\"$glcodefr11\" AND glcode<=\"$glcodeto11\") ORDER BY date ASC";
        $result166=$dbh2->query($res16query);
        ...
        $res16query=""; $result16=""; $found16=0; $ctr16=0;
        $res16query="SELECT acctspayableid, acctspayablenumber, debitamt, creditamt FROM tblfinacctspayable WHERE (date>=\"$datefrom\" AND date<=\"$dateto\") AND (glcode>=\"$glcodefr11\" AND glcode<=\"$glcodeto11\") ORDER BY date ASC";
