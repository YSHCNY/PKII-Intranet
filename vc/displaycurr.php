...
<?php
	...
	echo "<input type='hidden' name='eid' value='$employeeid0'>";
		...
		while($cutstart <= $cutend) {
			...
			<div  class="p-5 border shadow rounded-4 mx-auto my-5">
				
			</div>


<div class="p-5 border shadow rounded-4 mx-auto my-5" id = 'displaycard'>
...
// wfh ============================================================================================================================================
    ...
    $wfhflag="On Site";
        ...
        $wfhflag="";
                                                                        ...
                                                                        echo "<p $actrowspan > <span class='mainboxbg rounded-3 px-3 py-2 text-white'>$wfhflag</span></p>";
?>
    ...
    if(strtotime($timein14aval)!=0 && strtotime($timeout15aval)!=0) {
		...
		if($actrowctr>1) {
			...
			if($proj_sname16Arr[$x]=='') {
				...
				echo "<p class = 'text-info'>".$proj_sname16Arr[$x]."</p>";
			} //if-else
			} //if
...
<?php
			...
			echo "&nbsp;&nbsp;<button type=\"button\" class=\"btn btn-success btn-sm \" onclick=\"javascript:window.location='index.php?lst=1&lid=$loginid&sess=$session&p=143&aid=".$hractlogid16Arr[$x]."'\"><svg width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">
			...
			echo "&nbsp;&nbsp;<button type=\"button\" class=\"btn btn-danger btn-sm  btnActlogDelete\" data-toggle=\"modal\" data-target=\"#myModal\" data-id='".$hractlogid16Arr[$x]."'><svg width=\"16\" height=\"16\" viewBox=\"0 0 16 16\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">
		...
		echo "<td class='text-end'>";
			if($timestart16Arr[$x]!='' && $timeend16Arr[$x]!='') {
				...
				// echo "<strong>".number_format($timedur, 2)."</strong><br>man-hrs/task";
                                echo "<span class = 'text-muted fs-5'>Work Hours: </span><strong>".number_format($timedur, 2)."</strong><br>";
				} //if
			} // if
...
require '../includes/dbh.php';
		...
		if($x==0 && $actrowctr>1) {
		echo "<td $actrowspan><span class = 'text-muted fs-5'>Hours Redered: </span><input size='5' type='number' step='any' name='timeval[]' value='$timevaldaily'><br>";
        // 20210709 disp time duration from e-door log if exists
        if($timeduredr!=0) {
        echo "<span class = 'text-muted fs-5'>e-Door sum: </span>(<strong>$timeduredr</strong>&nbsp;fr&nbsp;office e-door log)<br>";
        } //if
