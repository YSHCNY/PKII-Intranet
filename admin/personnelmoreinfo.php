...
if ($found == 1) {
	...
	echo "<style>
		  ...
		  ?>
<div class="mb-4">
            <a href="<?php echo 'personnel.php?loginid=' . $loginid ?>" class="text-white mainbtnclr btn">Back</a>
    </div>
...
if($pid == '') {
	...
	if($result->num_rows>0) {
		...
		$term_resign = $myrow['term_resign'];
            $regularizationdt = $myrow['regularizationdt'];
        // $pid = $employeeid;
		if($picfn != "") { 
			echo "<div class = 'text-center'><img src=\"../vc/images/$picfn\" class = 'border rounded-circle' height=\"200\" width = '200'></div>";
			} else { echo "<div class = 'text-center'><img src=\"images/default.png\" class = 'border rounded-circle' height=\"200\" width = '200'></div>"; }
	
	

         echo"<div class = 'px-4 py-2 mt-3'>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Name: </p> <h5 class = 'mt-0 pt-0'>$name_first $name_middle $name_last ($pid)</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Sex: </p> <h5 class = 'mt-0 pt-0'>$contact_gender</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Address: </p> <h5 class = 'mt-0 pt-0'>$contact_address1 $contact_address2 $contact_city $contact_province $contact_zipcode $contact_country</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Landline: </p> <h5 class = 'mt-0 pt-0'>$num_res1_cc $num_res1_ac $num_res1</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'></p> <h5 class = 'mt-0 pt-0'>$num_res2_cc $num_res2_ac $num_res2</h5></div>";

		 if ($num_mobile1_cc == '' && $num_mobile1 == '' && $num_mobile1_ac  = '' ){
			$cutter = '';

		 } else if ($num_mobile2_cc == '' && $num_mobile2_ac == '' && $num_mobile2 == ''){
			$cutter2 = '';

		 } else if ($num_mobile3_cc == '' && $num_mobile3_ac == '' && $num_mobile3 == ''){
			$cutter3 = '';
         ...
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Mobile:</p> <h5 class = 'mt-0 pt-0'>$num_mobile1_cc $num_mobile1_ac $num_mobile1 $cutter $num_mobile2_cc $num_mobile2_ac $num_mobile2 $cutter2  $num_mobile3_cc $num_mobile3_ac $num_mobile3</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Email:</p> <h5 class = 'mt-0 pt-0'><a href=mailto:'$email1'>$email1</a>, <a href=mailto:'$email2'>$email2</a></h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Website:</p> <h5 class = 'mt-0 pt-0'>$url</h5></div>";
         echo "<div class = ''><p class = 'text-secondary pb-0 mb-0'>Remarks:</p> <h5 class = 'mt-0 pt-0'>$remarks_contact</h5></div>";
		 ...
		 echo "<div class = 'shadow border m-2 col p-4'>";
	echo "<div class ='text-center'><h4 class = 'text-secondary'>Employment Details</h4></div>";
		 
		 echo "<table class = 'table table-bordered table-striped table-hover'>";

         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Employee Reference Number</td><td>$emp_ref_num</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Date Hired</td><td>".date("Y-M-d", strtotime($date_hired))."</td></tr>";
//         echo "<tr><td>Date Expired</td><td>$date_expired</td></tr>";

        // 20240610 disp regularization date if date is valid
        if($regularizationdt=="" || $regularizationdt=="0000-00-00" || $regularizationdt=="0000-01-01") {
        // echo "<tr><td>Regularization date</td><td></td></tr>";
        } else {
        echo "<tr><td class = 'text-secondary w-25' align = 'right'>Regularization date</td><td>".date("Y-M-d", strtotime($regularizationdt))."</td></tr>";
        } //if

	if ($term_resign != '0000-00-00')
	{
	 echo "<tr><td class = 'text-secondary w-25' align = 'right'>Term Ended</td><td>$term_resign</td></tr>";
	}

        //20240610 re-hire history
// check if personnel has reemployment data
	$res12query = "SELECT daterehired, dateresigned, remarks FROM tblemprehire WHERE employeeid=\"$pid\" ORDER BY daterehired DESC";
	...
	if($found12 == 1) {
	  echo "<tr><td class = 'text-secondary w-25' align = 'right'>Re-hired or Re-employed History</td><td>";


	  echo "<table class = 'table table-striped table-bordered '>";
	  echo "<tr><td class='font-italic'><small>From</small></td><td class='font-italic'><small>To</small></td><td class='font-italic'><small>Remarks</small></td></tr>";
	  $res14query = "SELECT emprehireid, daterehired, dateresigned, remarks FROM tblemprehire WHERE employeeid=\"$pid\" ORDER BY daterehired DESC";
	    ...
	    echo "<tr><td>".date("Y-M-d", strtotime($daterehired14))."</td>";
			...
			echo "<td>$dateresigned14</td><td>$remarks14</td></tr>";
			} else {
			echo "<td>".date("Y-M-d", strtotime($dateresigned14))."</td><td>$remarks14</td></tr>";
			} // if($dtresignyyyy=="0000")
			} // while($myrow14=$result14->fetch_assoc())
		} // if($result14->num_rows>0)
	  ...
	  echo "</td></tr>";
         ...
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Birthdate</td><td>".date("Y-M-d", strtotime($emp_birthdate))."</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Birthplace</td><td>$emp_birthplace</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Civil Status</td><td>$emp_civilstatus</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>BIR TIN</td><td>$emp_tin</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>SSS</td><td>$emp_sss</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Philhealth</td><td>$emp_philhealth</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Pag-IBIG</td><td>$emp_pagibig</td></tr>";
				if($emp_pagibig2!='') {
					echo "<tr><td>Pag-IBIG2</td><td>$emp_pagibig2</td></tr>";
				} // if($emp_pagibig2!='')
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>GSIS</td><td>$emp_gsis</td></tr>";

         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Skills</td><td>$emp_skills</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Employment Status</td><td>"; // $emp_status
	  if($emp_status == 'R') {
	    echo "Regular";
        ...
        echo "</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Remarks</td><td>$emp_remarks</td></tr>";
         echo "<tr><td class = 'text-secondary w-25' align = 'right'>Type</td><td>$employee_type</td></tr>";
...
//	Start Employment details
	...
	if($result3->num_rows>0) {
	  ...
	  echo "<tr><td class = 'text-secondary w-25' align = 'right'>Position</td><td>";
		...
		echo "</td></tr>";
	  echo "<tr><td class = 'text-secondary w-25' align = 'right'>Department</td><td>$department</td></tr>";
	  echo "<tr><td class = 'text-secondary w-25' align = 'right'>Position Level</td><td>$positionlevel</td></tr>";
	  echo "<tr><td class = 'text-secondary w-25' align = 'right'>Salary grade</td><td>$salarygrade</td></tr>";
		} // while($myrow3=$result3->fetch_assoc())
	} // if($result3->num_rows>0)
...
echo "<div class = 'shadow border  my-5  p-5 '>";

	echo "<div class = 'mt-4'><h5>Project Assignment(s)</h5></div>";
	echo "<table class='table table-striped table-hover table-bordered' id='mngproj2'>
	<thead>
		<tr>
			<th class='text-secondary'>Reference Number</th>
			<th class='text-secondary'>Project Code - Name</th>
			<th class='text-secondary'>Position</th>
			<th class='text-secondary'>Project Date</th>
			<th class='text-secondary'>Duration</th>
		</tr>
	</thead>
	<tbody>";

$res2query = "SELECT ref_no, projassignid, projdate, proj_code, proj_name, position, durationfrom, durationto, idhrpositionctg 
		  FROM tblprojassign WHERE employeeid='$pid' ORDER BY durationfrom DESC";
...
while ($myrow2 = $result2->fetch_assoc()) {
	...
	echo "<tr>";
	echo ($accesslevel >= 5) ? 
		"<td><a href='personnelmoreinfoproj.php?loginid=$loginid&eid=$pid&prjid=$projectid' target='_blank'>$ref_no</a></td>" : 
		"<td>$ref_no</td>";
	
	echo "<td>" . (($proj_code != 'Select Pro') ? "$proj_code - " : "");
	echo !empty($proj_sname3) ? $proj_sname3 : (!empty($proj_fname3) ? substr($proj_fname3, 0, 20) : $proj_name);
	
	// Check related project assignments
	$res5aquery = "SELECT projcode, projname FROM tblprojcdassign WHERE projassignid=$projectid AND empid='$pid'";
	$result5a = $dbh2->query($res5aquery);
	while ($myrow5a = $result5a->fetch_assoc()) {
		echo " {$myrow5a['projcode']} - {$myrow5a['projname']}";
	}
	echo "</td>";
	
	// Retrieve position category
	$res19query = "SELECT name FROM tblhrpositionctg WHERE idhrpositionctg=$idhrpositionctg2";
	$result19 = $dbh2->query($res19query);
	$position_name = ($result19->num_rows > 0) ? $result19->fetch_assoc()['name'] : $position;
	echo "<td>$position_name</td>";
	
	echo "<td>$projdate</td>";
	echo "<td>" . (($durationfrom != '0000-00-00') ? date('Y-M-d', strtotime($durationfrom)) : "") . " - ";
	echo (($durationto != '0000-00-00') ? date('Y-M-d', strtotime($durationto)) : "") . "</td>";
	echo "</tr>";
}
}
...
// 	echo "<table class = 'table table-striped  table-hover table-bordered ' id = 'mngproj2'>";
// 	echo "<thead><tr><th class = 'text-secondary'>Reference Number</th>
// 	<th class = 'text-secondary'>Project Code - Name</th>
// 	<th class = 'text-secondary'>Position</th>
// 	<th class = 'text-secondary'>Project Date</th>

// 	<th class = 'text-secondary'>Duration</th></tr></thead>";
	
// echo "<tbody>";
// 	$res2query = "SELECT ref_no, projassignid, projdate, proj_code, proj_name, position, durationfrom, durationto, idhrpositionctg FROM tblprojassign WHERE employeeid='$pid' ORDER BY durationfrom desc";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Temporary Project Assignment(s)</h5></div>";




echo "<table class = 'table table-striped table-hover table-bordered ' id = 'tmpprojass2'>";
echo "<thead><tr><th class = 'text-secondary'>Reference Number</th>
<th class = 'text-secondary'>Project Code - Name</th>
<th class = 'text-secondary'>Position</th>
<th class = 'text-secondary'>Duration</th></tr></thead>";
echo "<tbody>";
	...
	if($result9->num_rows>0) {
	  ...
	  echo "<tr><td>$ref_no</td>
	  <td>$proj_name</td>
	  <td>$position</td>
	  <td>$durationfrom -to- $durationto</td>
	  </tr>";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Insurance details</h5></div>";

	echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
	echo "<tr><td align=center>InsuranceName</td><td align=center>GroupPolicy#</td><td align=center>EmployeePolicy#</td><td align=center>From</td><td align=center>To</td><td align=center>Location</td></tr>";
...
// start prof license details
	...
	echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Professional License details</h5></div>";
	echo "<table class = 'table table-striped table-hover table-bordered '>";
	echo "<tr><td align=center>Regulatory Board</td><td align=center>Profession</td><td align=center>License Number</td><td align=center>Date</td></tr>";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Passport details</h5></div>";
	echo "<table class = 'table table-striped table-hover table-bordered '>";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Educational Background</h5></div>";

	echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
	echo "<tr><td align=center>Course</td><td align=center>YearGraduated</td><td align=center>School/University</td></tr>";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>Bank Account Details</h5></div>";

	echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
	echo "<tr><td align=center>BankName</td><td align=center>Branch</td><td align=center>AcctNumber</td><td align=center>Type</td><td align=center>Currency</td><td align=center>AcctName</td><td align=center>PayrollAcct</td></tr>";
...
// start emergency contact details
	...
	echo "<div class = 'shadow border  my-5  p-5 '>";
	echo "<div class = 'mt-4'><h5>Emergency Contact Information</h5></div>";
	
		echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
     echo "<tr><td align=center>Name</td><td align=center>Relation</td><td align=center>Landline</td><td align=center>Mobile</td><td align=center>Email</td></tr>";
...
// start spouse details
  if ($emp_civilstatus <> "Single") {
	...
	echo "<div class = 'shadow border  my-5  p-5 '>";
	echo "<div class = 'mt-4'><h5>Spouse details</h5></div>";
	
		echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
	echo "<tr><td align=center>Name</td><td align=center>Birthdate</td></tr>";
...
echo "<div class = 'shadow border  my-5  p-5 '>";
echo "<div class = 'mt-4'><h5>List of Dependents</h5></div>";

	echo "<tr><td colspan=2><table class = 'table table-hover table-striped table-bordered '>";
	echo "<tr><td align=center>Name</td><td align=center>Birthdate</td><td align=center>Relationship</td></tr>";
