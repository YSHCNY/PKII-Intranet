...
if ($found == 1)
{
  ...
  {
		// insert new record
		$result11 = mysql_query("UPDATE tblfinnkgacctref SET timestamp=\"$now\", loginid=$loginid, datecreated=\"$datenow\", type=\"$nkstrvstype\", code=\"$nkstrvscode\", name_j=\"$nkstrvsnamej\", name_e=\"$nkstrvsnamee\", seq=$nkstrvsseq, formref=\"$nkstrvsformref\", sheetref=\"$nkstrvssheetref\", tabpos=$nkstrvstabpos, remarks=\"$nkstrvsremarks\" WHERE finnkgacctrefid=$finnkgacctrefid", $dbh);

    // create log
    $result16 = mysql_query("SELECT adminuid FROM tbladminlogin WHERE adminloginid=$loginid", $dbh);
    while($myrow16 = mysql_fetch_row($result16))
    { $adminuid16=$myrow16[0]; }
    $adminlogdetails = "$loginid:$adminuid16 - modified NK-Stravis acct code:$nkstrvscode name:$nkstrvsnamej-$nkstrvsnamee formsheet:$nkstrvsformref-$nkstrvssheetref";
