...
?>
	<div class="bg-white">
		<div  class="">
			<p class="text-muted fs-4 "><?php echo "<span class = 'fw-bold'>$department014</span>"; ?> Schedules</p>
		</div>
		<?php
			...
			$res012query = "SELECT idtblscheduler, loginid, lastupdate, schedname, datefrom, dateto, details, recurring, deptcd, notifysw, notifywhen, notifywho, displaywhere FROM tblscheduler WHERE (datefrom >= \"$datenow\" AND dateto <= \"$datenowplus2mo\") OR (DATE_FORMAT(datefrom, '%y-%m-%d') >= DATE_FORMAT('$datenow', '%m-%d') AND DATE_FORMAT(datefrom, '%m-%d') <= DATE_FORMAT('$datenowplus2mo', '%m-%d') AND recurring=1) AND deptcd LIKE \"%$department014%\" ORDER BY DATE_FORMAT(datefrom, '%m-%d') ASC LIMIT 10"; //upd20250514
					...
					<div class="border px-3 pt-4 pb-3 rounded-4 mb-3 ">
						<div id="" class="text-center">
							...
							if ($datefrom012 == $dateto012) {
								echo "<p class = 'text-danger fw-bold fs-4 '>$schedname012</p>";
							} else {
								echo "<p class = 'text-danger fw-bold fs-4 '>" . date("M d, Y (D)", strtotime($datefrom012)) . " " . date("D Y-M-d", strtotime($dateto012)) . "</p>";
							}
								echo "<p class = 'fs-4 '>" . date("M d, Y (D)", strtotime($datefrom012)) . "</p>";
							?>
						</div>
					</div>
					<?php
			...
			?>
				<div class="border rounded-4 p-5 text-center ">
					<p class="text-danger fs-4 fw-medium  m-0 py-1">No Upcoming Schedules</p>
				</div>
			<?php
			}
		?>
