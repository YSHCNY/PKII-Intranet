...
if($found == 1) {
	...
	if($found11 == 1) {
		echo "<p><strong><font color=\"red\">Warning:&nbsp;cutoff&nbsp;[".date('Y-M-d', strtotime($cutstart))." -to- ".date('Y-M-d', strtotime($cutend))."]&nbsp;exists.</strong></font></p>";
		if($status11==0) {
			echo "<p><strong>An inactive or deleted cut-off is detected.</strong><br>Press <strong>[Yes]</strong> to restore/regenerate its records.<br>Press <strong>[No]</strong> to cancel.</p>";
			echo "<p>";
...
?>
	<script>
	// Redirect to the new page using existing PHP variables
window.location.href = 'hrtimeattcutoff.php?loginid=' + encodeURIComponent(<?php echo json_encode($loginid); ?>) + '&idpg=' + encodeURIComponent(<?php echo json_encode($idhrtapaygrp); ?>);
