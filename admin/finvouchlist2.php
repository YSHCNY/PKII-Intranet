...
//
  ...
  echo "<td><input name=\"searchcv\" size=\"20\" value=\"$searchcv\">";
	...
	echo "<option value=\"payee\" $cvsrchtyppayee>payee/payor</option>";
  ...
  echo "<tr><th>Count</th><th>Date</th><th>C.V. No.</th><th>Payee</th><th>Particulars</th>";
	// echo "<th>Type</th>";
	echo "<th>DebitTotal</th><th>CreditTotal</th><th>Status</td><th colspan=\"2\">Action</th></tr>";

  if($cvtype == 'all')
  {
      $result11 = mysql_query("SELECT DISTINCT disbursementnumber, disbursementtype, date, companyid, contactid, explanation FROM tblfindisbursement WHERE disbursementid<>'' AND DATE_FORMAT(date, '%Y %M') = \"$yrmonthavlbl\" ORDER BY date DESC, disbursementnumber DESC", $dbh);
  }
  else if($cvtype == 'cv')
  {
      $result11 = mysql_query("SELECT DISTINCT disbursementnumber, disbursementtype, date, companyid, contactid, explanation FROM tblfindisbursement WHERE disbursementid<>'' AND disbursementnumber NOT LIKE '%DM%' AND DATE_FORMAT(date, '%Y %M') = \"$yrmonthavlbl\" ORDER BY date DESC, disbursementnumber DESC", $dbh);
  }
  else if($cvtype == 'dm')
  {
      $result11 = mysql_query("SELECT DISTINCT disbursementnumber, disbursementtype, date, companyid, contactid, explanation FROM tblfindisbursement WHERE disbursementid<>'' AND disbursementnumber LIKE '%DM%' AND DATE_FORMAT(date, '%Y %M') = \"$yrmonthavlbl\" ORDER BY date DESC, disbursementnumber DESC", $dbh);
  }
...
//    $result11 = mysql_query("SELECT DISTINCT tblfindisbursement.disbursementnumber, tblfindisbursement.disbursementtype, tblfindisbursement.payee, tblfindisbursement.date FROM tblfindisbursement LEFT JOIN tblfindisbursementtot ON tblfindisbursement.disbursementnumber = tblfindisbursementtot.disbursementnumber WHERE tblfindisbursement.disbursementnumber LIKE \"%$searchcv%\" OR tblfindisbursement.projcode LIKE \"%$searchcv%\" OR tblfindisbursement.particulars LIKE \"%$searchcv%\" OR tblfindisbursement.payee LIKE \"%$searchcv%\" OR tblfindisbursementtot.explanation LIKE \"%$searchcv%\" ORDER BY tblfindisbursement.date DESC, tblfindisbursement.disbursementnumber DESC", $dbh);
	...
	$result11 = mysql_query("SELECT DISTINCT tblfindisbursement.disbursementnumber, tblfindisbursement.disbursementtype, tblfindisbursement.date, tblfindisbursement.companyid, tblfindisbursement.contactid, tblfindisbursement.explanation FROM tblfindisbursement WHERE disbursementnumber=\"$disbursementnumber14\" ORDER BY date DESC, disbursementnumber DESC", $dbh);
  ...
  {
    echo "<tr><td colspan=\"5\" align=\"right\"><b>Total ASASAS";
...
//    $result11 = mysql_query("SELECT DISTINCT acctspayablenumber, payee, date FROM tblfinacctspayable LEFT JOIN tblfinacctspayabletot ON tblfinacctspayable.acctspayablenumber = tblfinacctspayabletot.acctspayablenumber WHERE tblfinacctspayable.acctspayablenumber LIKE \"%$searchap%\" OR tblfinacctspayable.projcode LIKE \"%$searchap%\" OR tblfinacctspayable.particulars LIKE \"%$searchap%\" OR tblfinacctspayable.payee LIKE \"%$searchap%\" OR tblfinacctspayabletot.explanation LIKE \"%$searchap%\" ORDER BY tblfinacctspayable.date DESC, tblfinacctspayable.acctspayablenumber DESC", $dbh);
  ...
  if($debitmonthtot != 0 && $creditmonthtot != 0)
  {
    echo "<tr><td colspan=\"4\" align=\"right\"><b>Total CRVCS</b></td><td><b>".number_format($debitmonthtot,2)."</b></td><td><b>".number_format($creditmonthtot,2)."</b></td><td colspan=\"3\">&nbsp;</td></tr>";
  }
  echo "</table>";
?>
</div>
...
// end filtering by available months
  ...
  echo "<tr><th>Count</th><th>Date</th><th>C.R. No.</th><th>Received from</th><th>Particulars</th><th>DebitTotal</th><th>CreditTotal</th><th>Status</th><th colspan=\"2\">Action</th></tr>";
  $result11 = mysql_query("SELECT DISTINCT cashreceiptnumber, date, companyid, contactid, explanation FROM tblfincashreceipt WHERE cashreceiptid<>'' AND DATE_FORMAT(date, '%Y %M') = \"$yrmonthavlbl\" ORDER BY date DESC, cashreceiptnumber DESC", $dbh);
  if($searchcr != "") {
//    $result11 = mysql_query("SELECT DISTINCT tblfincashreceipt.cashreceiptnumber, tblfincashreceipt.date FROM tblfincashreceipt LEFT JOIN tblfincashreceipttot ON tblfincashreceipt.cashreceiptnumber = tblfincashreceipttot.cashreceiptnumber WHERE tblfincashreceipt.cashreceiptnumber LIKE \"%$searchcr\" OR tblfincashreceipt.projcode LIKE \"%$searchcr%\" OR tblfincashreceipt.particulars LIKE \"%$searchcr%\" OR tblfincashreceipttot.explanation LIKE \"%$searchcr%\" ORDER BY tblfincashreceipt.date DESC, tblfincashreceipt.cashreceiptnumber DESC LIMIT 0, 100", $dbh);
    ...
    while($myrow14 = mysql_fetch_row($result14)) {
      ...
      $result11 = mysql_query("SELECT DISTINCT cashreceiptnumber, payor, date, companyid, contactid FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber14\" ORDER BY date DESC", $dbh);
  if($result11 != '') {
  while($myrow11 = mysql_fetch_row($result11))
  {
    $found11 = 1;
  ...
  if($debitmonthtot != 0 && $creditmonthtot != 0)
  {
    echo "<tr><td colspan=\"4\" align=\"right\"><b>Total dsaxczcz</b></td><td align=\"right\"><b>".number_format($debitmonthtot,2)."</b></td><td align=\"right\"><b>".number_format($creditmonthtot,2)."</b></td><td colspan=\"4\">&nbsp;</td></tr>";
  }
...
//    $result11 = mysql_query("SELECT DISTINCT tblfinjournal.journalnumber, tblfinjournal.date FROM tblfinjournal LEFT JOIN tblfinjournaltot ON tblfinjournal.journalnumber = tblfinjournaltot.journalnumber WHERE tblfinjournal.journalnumber LIKE \"%$searchjv%\" OR tblfinjournal.projcode LIKE \"%$searchjv%\" OR tblfinjournal.particulars LIKE \"%$searchjv%\" OR tblfinjournaltot.explanation LIKE \"%$searchjv%\" ORDER BY tblfinjournal.date DESC, tblfinjournal.journalnumber DESC", $dbh);
  ...
  if($debitmonthtot != 0 && $creditmonthtot != 0)
  {
    echo "<tr><td colspan=\"3\" align=\"right\"><b>Total dsadasds</b></td><td><b>".number_format($debitmonthtot,2)."</b></td><td><b>".number_format($creditmonthtot,2)."</b></td><td colspan=\"3\">&nbsp;</td></tr>";
  }
