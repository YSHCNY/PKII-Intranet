...
*/
  ...
  $result12 = mysql_query("UPDATE tblfindisbursement SET disbursementnumber=\"$cvnumber\", disbursementtype=\"$disbursementtype\", payee=\"$cvpayee\", date=\"$cvdate\", glcode=\"$glcode\", glrefver=\"$version17\", glnamedetails=\"$glnamedetails\", projcode=\"$projcode\", particulars=\"$allparticulars\", debitamt=\"$debitamt\", creditamt=\"$creditamt\" WHERE disbursementid=$disbursementid AND disbursementnumber=\"$cvnumber\"", $dbh);
...
//  $credittot = $credittot11 + $creditamt;
	...
	$result16=""; $found16=0;
  $result16 = mysql_query("SELECT disbursementtotid, explanation FROM tblfindisbursementtot WHERE disbursementnumber=\"$cvnumber\"", $dbh);
  ...
  if($found16 == 1)
  {
    $result14 = mysql_query("UPDATE tblfindisbursementtot SET date=\"$cvdate\", debittot=$debittot11, credittot=$credittot11, status=\"$status\" WHERE disbursementtotid=$disbursementtotid16 AND disbursementnumber=\"$cvnumber\"", $dbh);
  }
  else
  {
    $result15 = mysql_query("INSERT INTO tblfindisbursementtot SET disbursementnumber=\"$cvnumber\", date=\"$cvdate\", explanation=\"$explanationfin\", debittot=$debittot11, credittot=$credittot11, status=\"$status\"", $dbh);
  }
...
// create log
    ...
    while($myrow16 = mysql_fetch_row($result16))
    { $adminuid=$myrow16[0]; }
    $adminlogdetails = "$loginid:$adminloginuid - Modified CheckVoucher item with CV:$cvnumber - details:$allparticulars, acctcode:$glcode, debit:$debitamt, credit:$creditamt";
