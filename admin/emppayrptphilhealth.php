...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"emppayrptphilhealth.php?loginid=$loginid&submitsw=1\" method=\"post\" name=\"emppayrptphilhealth\">";
	...
	// if(($datefrom <= $dateto) && (($datefrom != '') && ($dateto != ''))) {
		...
		echo "<tr><th colspan=\"2\" align=\"left\">Employees Payroll - Philhealth&nbsp;<a href=\"#\" id=\"exportToExcel\"><img src=\"./images/sheet.gif\"></a></th></tr>";
		...
		echo "<tr><td colspan=\"2\">Note: Philhealth Contribution Table reference dated 2014. Archived schedules may have different output values.</td></tr>";
    echo "<tr><td colspan=\"2\">";
		...
		echo "<th>EE<br>Ded<br>".date("M-d", strtotime($cut_start12))."-to-".date("d", strtotime($cut_end12))."<br>MED</th>";
		echo "<th>Cont.<br>MED</th>";
		echo "<th>Short<br><font color=\"red\">(Over)</font><br>Ded<br>MED</th>";
		echo "<th>ERR<br>Share<br>MED</th>";
		echo "<th>TOTAL<br>CONT.<br>(EE+ERR)</th></tr>";
		...
		if($result14 != "") {
			...
			if($result15 != "") {
				...
				$netbasicpaytot = $netbasicpaytot + $netbasicpay;
				...
				$netbasicpay=0; $payrate=0; $totallateabsent=0;

				}
			}

			echo "<th>".number_format($netbasicpaytot, 2)."</th>";

			// echo "<td align=\"right\">".number_format($phil_ded16, 2)."</td>";
			echo "<td align=\"right\">".number_format($philemp16, 2)."</td>";

			$result17=""; $found17=0; $ctr17=0;
			$result17 = mysql_query("SELECT salaryid, salarymin, salarymax, salarybase, totmonthpremium, philhealther, philhealthee FROM tblphilhealth WHERE salarymin<=$netbasicpaytot AND salarymax>=$netbasicpaytot", $dbh);
			if($result17 != "") {
				while($myrow17 = mysql_fetch_row($result17)) {
				$found17 = 1;
				$salaryid17 = $myrow17[0];
				$salarymin17 = $myrow17[1];
				$salarymax17 = $myrow17[2];
				$salarybase17 = $myrow17[3];
				$totmonthpremium17 = $myrow17[4];
				$philhealther17 = $myrow17[5];
				$philhealthee17 = $myrow17[6];
								
				}
			}
			echo "<td align=\"right\">".number_format($philhealthee17, 2)."</td>";

			// $shortded = $philhealthee17 - $phil_ded16;
			$shortded = $philhealthee17 - $philemp16;
			...
			echo "<td align=\"right\">".number_format($philhealther17, 2)."</td>";

			$totalcontrib = $philhealthee17 + $philhealther17;
			echo "<th align=\"right\">".number_format($totalcontrib, 2)."</th>";
			echo "</tr>";
			$empidtmp = $employeeid14;
			$netbasicpaytot=0; $shortded=0; $totalcontrib=0;
			}
		}
