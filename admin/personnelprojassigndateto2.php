...
if ($found == 1)
{
     ...
     echo "<p><font size=1>Directory >> Manage Personnel >> Edit Project Assignment >> Change date_to project duration</font></p>";
...
//	  $employeeid = $myrow0[0];
	...
	echo "<p>Duration end date changed for: <b>$employeeid - $name_last, $name_first $name_middle[0]</b></p>";

	$result = mysql_query("UPDATE tblprojassign SET durationto = '$newdate'
		WHERE employeeid='$employeeid' AND projassignid = '$projassignid'", $dbh) or die ("Couldn't execute query.".mysql_error());

	echo "durationto = $newdate<br>";
