<?php
//
// mngadmmtgrmeqptndctgadd.php //20220104
...
if($code!='' && $name!='') {
	...
	$res11query="SELECT idadmctgmtgrmeqptlst FROM tbladmctgmtgrmeqptlst WHERE code='$code' OR name='$name'";
	...
	echo "<p><a href='mngadmmtgrmeqptndctg.php?loginid=$loginid' class='btn btn-default'>back</a></p>";
	} else {
	//proceed
	$res12query="INSERT INTO tbladmctgmtgrmeqptlst SET `timestamp`=\"$now\", `loginid`=$loginid, `code`=\"$code\", `name`=\"$name\", `description`=\"$description\", `remarks`=\"$remarks\"";
	$result12=$dbh2->query($res12query);
	// log
	$logdetails="Add new category in tbladmctgmtgrmeqptlst. code:$code name:$name";
	...
	// halt
	echo "<h3 class='text-danger'>Sorry. Blank form not allowed.</h3>";
	echo "<p><a href='mngadmmtgrmeqptndctg.php?loginid=$loginid' class='btn btn-default'>back</a></p>";
