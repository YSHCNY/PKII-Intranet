...
$filesrc0 = (isset($_GET['fsrc'])) ? $_GET['fsrc'] :'';
...
$idhrtaempincome15 = (isset($_GET['empincid'])) ? $_GET['empincid'] :'';
...
// query allowances under projassignid
	...
	$allowtyp = (isset($_POST['typeofallow'])) ? $_POST['typeofallow'] : '';
...
if($found == 1) {
	...
	*/
	...
	$res14query = "SELECT tblcontact.name_last, tblcontact.name_first, tblcontact.name_middle, tblcontact.contact_gender FROM tblhrtapaygrpemplst INNER JOIN tblcontact ON tblhrtapaygrpemplst.contactid=tblcontact.contactid WHERE tblhrtapaygrpemplst.employeeid=\"$employeeid\" AND tblcontact.contact_type=\"personnel\"";
	$result14=""; $found14=0;
	/*
	...
	*/
	...
	if($found11 == 1 && $employeeid != "" && $incomename != "" && $amount>0 && $dateend>=$datestart) {
		...
		$res12query = "UPDATE tblhrtaempincome SET timestamp=\"$now\", lastupdate=$loginid, paygroupname=\"$paygroupname11\", employeeid=\"$employeeid\", name=\"$incomename\", amount=\"$amount\", datestart=\"$datestart\", dateend=\"$dateend\", nontaxable=$nontaxableval, vatinclusive=$vatinclusiveval, status=$statusval, schedule=\"$schedule\", idpaygroup=$idpaygroup, projassignid=$projassignid, allowtyp=\"$allowtyp\", lumpsumsw=\"$lumpsumsw\", amtlumpsumtotal=$amtlumpsumtotal, amtlumpsumbalance=$amtlumpsumbalance WHERE idhrtaempincome=$idhrtaempincome";
		$result12 = $dbh2->query($res12query);
		if ($result12 == TRUE){
			$_SESSION['success_message'] = 'Additional Allowance Updated Successfully!';
			header("Location: hrtimeattincomeedit.php?loginid=$loginid&empincid=$idhrtaempincome15&fsrc=$filesrc&idpg=$idpaygroup&tab=$tab");
			

		} else {
			$_SESSION['error_message'] = 'Additional Allowance Update Denied!';
			header("Location: hrtimeattincomeedit.php?loginid=$loginid&empincid=$idhrtaempincome15&fsrc=$filesrc&idpg=$idpaygroup&tab=$tab");
		}
		// create log
  	$res16query = "SELECT adminuid FROM tbladminlogin WHERE adminloginid=$loginid";
		...
		$adminlogdetails = "$loginid:$adminuid - modified additional income for personnel: $employeeid - $name_last14, $name_first14 $name_middle14 under paygroup:$idpaygroup-$paygroupname11 in payroll system with id:$idhrtaempincome income:$incomename amt:$amount dur:$datestart-to-$dateend, nontax:$nontaxable, vatinc:$vatinclusive, sched:$schedule, status:$status, projid:$projassignid, lumpsum:$lumpsumsw, lstot:$lumpsumtotal, lsbal:$lumpsumbalance";
	...
	// echo "<p><a href=\"$filesrc.php?loginid=$loginid&idpg=$idpaygroup&eid=$employeeid\">back</a></p>";
	}
