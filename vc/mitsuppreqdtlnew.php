...
<?php
    if($apprdurationsw16==1) {
		echo "<tr><th class=\"text-right\">Allowed duration of request</th><td class=\"text-left\">";
    echo "".date('Y-M-d', strtotime($apprdurationdt16))."";
		echo "</td></tr>";
    } //if
?>
...
<?php
	...
	if($approveempid16==$employeeid0) {
		$actor="APP";
		if($approvectr16==0) {
		echo "<form class='form-inline' method=\"POST\" action=\"mitsuppreq2.php?lst=1&lid=$loginid&sess=$session&p=342\" name=\"mitsuppreq2\">";
    echo "<div form-group>";
		echo "<input type=\"hidden\" name=\"idsr\" value=\"$iditsupportreq\">";
		echo "<input type=\"hidden\" name=\"ctgactor\" value=\"$actor\">";
    ...
    if($apprdurationdt16!="") { $apprdurationswdtval="".date(Y-M-d, strtotime($apprdurationdt16)).""; } else { $apprdurationswdtval=$datenow; }
		echo "<tr><th class=\"text-right\">(optional) Allowed duration of request</th><td class=\"text-left\">";
