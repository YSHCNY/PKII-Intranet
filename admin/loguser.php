...
<div class="shadow px-4 py-5 my-2">
  ...
  <div class="flex border rounded-3 p-4">
    ...
    <?php
      echo "<form method=\"post\" action=\"loguser.php?loginid=$loginid&page=1\" class=\"text-center mt-3\" name=\"formlog\">";
      echo "<span class=\"text-muted fs-4\">From</span> <input size=\"8\" class=\"my-2 me-2 bg-white rounded-3 px-3 py-2 border\" name=\"logdtfr\" type=\"date\" value=\"$logdtfr1\">";
      echo "<span class=\"text-muted fs-4\">To</span> <input size=\"8\" class=\"my-2 me-3 bg-white rounded-3 px-3 py-2 border\" name=\"logdtto\" type=\"date\" value=\"$datenow\">";
      ...
      if ($result12) {
          while ($myrow12 = mysql_fetch_row($result12)) {
              ...
              $logusrnmsel = ($username12 == $logusrnm) ? "selected" : "";
  ...
  <?php
    ...
    if (($logusrnm != "usersall") && ($logusrnm != "")) {
        $whereClause .= " AND username = '$logusrnm'";
    }
    $query = "SELECT timestamp, loginid, username, logdetails FROM tbllogs WHERE $whereClause ORDER BY timestamp DESC LIMIT $offset, $records_per_page";
          ...
          <div id='hov' class='border rounded-3 px-5 m-4'>  
            <div class='row pt-5'>
              <div class='col-lg-10'>
                ...
                <p class=''><i>On</i> $timestamp11, <span class='text-uppercase fw-bold'>$username11</span>, <i>$logdetails11</i></p>
              </div>
              <div class='col-lg-2'>
    ...
    $total_records_query = "SELECT COUNT(*) AS total_records FROM tbllogs WHERE $whereClause";
  ...
  <div class="pagination poppins d-flex justify-content-end gap-1">
      ...
      if ($total_pages <= 5) {
          ...
          if ($page > 3) {
              echo "<span class='btn border border-1 btn-outline-primary text-black fw-semibold'>...</span>";
          }
