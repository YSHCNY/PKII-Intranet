<?php
echo "<form method='POST' action='mactivitylogadd.php?lst=1&lid=" . $loginid . "&sess=" . $session . "&p=141'>";
?>
...
<div class="">
	...
	<div class="row">
		<div class = "form-group col-6 col-auto">
		<label for="actdate" class = 'text-secondary fw-normal'>Start Date</label>
		...
		<div class = "form-group col-6 col-auto">
		<label for="endactdate" class = 'text-secondary fw-normal'>End Date</label>
		<input type="date" size="10" class="form-control rounded-4 shadow-none bg-white border" name="endactdate" id='endactdate' value="<?php echo $datenow; ?>">
		</div>
	</div>
		<div class="form-group">
			<label for="actdetails" class = 'text-secondary fw-normal'>Details</label>
			<textarea style="height:100px;" placeholder="Type here..." class="form-control rounded-4 shadow-none bg-white border" name="actdetails" id='actdetails' required></textarea>
		</div>
...
?>
    ...
    <select class="form-control" name="prjcd" id='prjcd' onchange="displaySelectedValue()">
...
?>
	...
	<!-- <p id="displayValue">Selected Value: GAE</p>

<script>
function displaySelectedValue() {
    const selectElement = document.getElementById("prjcd");
    const selectedValue = selectElement.value;
    document.getElementById("displayValue").innerText = "Selected Value: " + selectedValue;
}
</script> -->
<!-- <div id="selectedOptionDisplay">No option selected yet.</div> -->
</div>

<script>
// Function to update the displayed value when an option is selected
const selectElement = document.getElementById("prjcd");
const display = document.getElementById("selectedOptionDisplay");

selectElement.addEventListener("change", () => {
    display.textContent = `You selected: ${selectElement.value}`;
});
</script>
...
<?php
	...
	echo "<select name='tmstarthh' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmstarthh'>";
	...
	echo "<select name='tmstartmm' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmstartmm'>";
	...
	echo "<select name='tmstartampm' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmstartampm'>";
...
<?php
	echo "<p class = 'text-secondary ms-5 ps-5 fw-normal'>Hour</p>";
	echo "<select name='tmendhh' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmendhh'>";
	echo "<option selected disabled >HH</option>";
	...
	echo "<p class = 'text-secondary ms-5 ps-5 fw-normal'>Minutes</p>";
	echo "<select name='tmendmm' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmendmm'>";
	echo "<option selected disabled >MM</option>";
	...
	echo "<p class = 'text-secondary ms-5 ps-5 fw-normal'>Time Period</p>";
	echo "<select name='tmendampm' class ='w-75 text-center bg-white form-control mx-auto m-3' id='tmendampm'>";
	echo "<option selected disabled >TT</option>";
	echo "<option  value='pm'>pm</option>";
	echo "<option value='am'>am</option>";
	echo "</select>";
...
?>
	...
	<div>
<button class="btn w-100 bg-success float-end  p-4 text-white" id='btnActdtlSubmit' type = 'submit'>Submit</button>
