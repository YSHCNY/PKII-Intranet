...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"emppayrptovertime.php?loginid=$loginid&submitsw=1\" method=\"post\" name=\"emppayrptbasic\">";
	...
	// if(($datefrom <= $dateto) && (($datefrom != '') && ($dateto != ''))) {
		...
		echo "<tr><th colspan=\"2\" align=\"left\">Employees Payroll - Overtime&nbsp;<a href=\"#\" id=\"exportToExcel\"><img src=\"./images/sheet.gif\"></a></th></tr>";
    ...
    echo "<tr><td colspan=\"2\">";
		...
		if($result14 != "") {
			...
			if($result15 != "") {
				...
				$result16 = mysql_query("SELECT regholidayamt, speholidayamt, otsundayamt, overamt, nightdiffamt FROM tblemppayroll WHERE employeeid=\"$employeeid14\" AND cut_start=\"$cut_start15\" AND cut_end=\"$cut_end15\"", $dbh);
				if($result16 != "") {
					while($myrow16 = mysql_fetch_row($result16)) {
					$found16 = 1;
					$regholidayamt16 = $myrow16[0];
					$speholidayamt16 = $myrow16[1];
					$otsundayamt16 = $myrow16[2];
					$overamt16 = $myrow16[3];
					$nightdiffamt16 = $myrow16[4];
					$empovertime = $regholidayamt16 + $speholidayamt16 + $otsundayamt16 + $overamt16 + $nightdiffamt16;
					}
				}

				$empovertimetot = $empovertimetot + $empovertime;

				if($found16 == 1) {
					echo "<td align=\"right\">".number_format($empovertime, 2)."</td>";
				} else {
					echo "<td></td>";
				}
				$empovertime=0;
			...
			echo "<th>".number_format($empovertimetot, 2)."</th></tr>";
