...
//	Start Display project(s) involvement
	...
	$res233query = "SELECT insuranceempid, policynum, insurancename, emppolicynum, durationfrom, durationto, location FROM tblinsuranceemp WHERE employeeid=\"$employeeid0\" ORDER BY durationto DESC, durationfrom DESC";
	$result233="";
	$result233=$dbh->query($res233query);
	if($result233->num_rows>0) {
		while($myrow233=$result233->fetch_assoc()) {
	  $insuranceempid = $myrow233['insuranceempid'];
	  $policynum = $myrow233['policynum'];
	  $insurancename = $myrow233['insurancename'];
	  $emppolicynum = $myrow233['emppolicynum'];
	  $durationfrom = $myrow233['durationfrom'];
	  $durationto = $myrow233['durationto'];
	  $location = $myrow233['location'];
	...
	$res10query = "SELECT * FROM tblempproflicense WHERE employeeid=$employeeid0";
