...
if ($found == 1)
{
     ...
     echo "<tr><th colspan=9>Manage Personnel Information</th></tr>";

     echo "<tr><td colspan=9>";

     echo "<table class=\"fin\">";
     echo "<tr><td><form action=personneladd.php?loginid=$loginid method=POST name=\"personneladd\">";
     echo "<input type=submit value=\"Add new record\"></form></td>";

     echo "<td><form action=personnelinsurance.php?loginid=$loginid method=POST name=\"personnelinsurance\">";
     echo "<input type=submit value=\"Manage Group Insurance\"></form></td>";

     echo "<td><form action=personneltmpprojassignmng.php?loginid=$loginid method=POST name=\"personneltmpprojassignmng\">";
     echo "<input type=submit value=\"Manage tmp.Proj.Assignments\"></form></td>";

//     echo "<td><form action=\"personnelinfomerge.php?loginid=$loginid\" method=\"POST\">";
//     echo "<input type=\"submit\" value=\"Merge Personnel Info\"></form></td>";
     ...
     echo "<tr><td colspan=9>";
     echo "<form action=personneledit.php?loginid=$loginid method=POST name=\"personneledit\">";
	...
	$resquery = "SELECT tblcontact.employeeid, tblcontact.name_first, tblcontact.name_last, tblcontact.name_middle, tblcontact.email1, tblcontact.email2, tblcontact.contact_address1, tblcontact.contact_address2, tblcontact.contact_city, tblcontact.contact_province, tblcontact.contact_zipcode, tblcontact.contact_country, tblcontact.num_res1_cc, tblcontact.num_res1_ac, tblcontact.num_res1, tblcontact.num_res2_cc, tblcontact.num_res2_ac, tblcontact.num_res2, tblcontact.num_mobile1_cc, tblcontact.num_mobile1_ac, tblcontact.num_mobile1, tblcontact.num_mobile2_cc, tblcontact.num_mobile2_ac, tblcontact.num_mobile2 FROM tblcontact JOIN tblemployee ON tblcontact.employeeid = tblemployee.employeeid WHERE tblcontact.contact_type = 'personnel' AND tblemployee.emp_record = 'inactive' ORDER BY $employeeorder";
     ...
     echo "<tr><td colspan=9>Displaying list: <b>$employeetype</b> in <b>$employeeorder</b> order.<br>";
     echo "Click 'Edit' on the 'Action' column to modify personnel information.</td></tr>";
     echo "<tr bgcolor=yellow><td>Count</td><td>EmpID</td><td>LastName</td><td>FirstName</td><td>M.I.</td><td>Landline</td><td>Mobile(s)</td><td>Email(s)</td><td>Action</td></tr>";
	...
	echo "</td>";
	echo "<td><a href = personneledit2.php?pid=$pid&loginid=$loginid>Edit</a></td></tr>";
		} // while($myrow = $result->fetch_assoc())
	} // if($result->num_rows>0)
