<?php
 ...
 // this->label for devenv only
          ...
          while($myrow = $result1->fetch_assoc()) {
            ...
            $btn = '';
            if($status == 'Pending'){
                $btn = "<button class='btn btn-success text-white btn-approve' data-id='".$myrow['idhrtaotreq']."'>Approve</button>";
                $btn .= "<button class='btn btn-danger text-white btn-disapprove' data-id='".$myrow['idhrtaotreq']."'>Disapprove</button>";
            }
            ...
            if($myrow['approverid'] == $employeeid0){
              echo "<td><a class='secondarybgc rounded-3 px-3 py-2 border-0 text-white btn-view' href='index.php?lst=1&lid=".$loginid."&sess=".$session."&p=364&otid=".$myrow['idhrtaotreq']."'>Details</a>".$btn."</td>";
            }
            else{
              echo "<td><a class='secondarybgc rounded-3 px-3 py-2 border-0 text-white btn-view' href='index.php?lst=1&lid=".$loginid."&sess=".$session."&p=364&otid=".$myrow['idhrtaotreq']."'>Details</a></td>"; 
            }
            echo "</tr>";
          }
