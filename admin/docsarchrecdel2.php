...
// start contents here
  ...
  if($filename10 != "") {
	...
	// delete record in tbldocsarchives
	$result11 = mysql_query("DELETE FROM tbldocsarchives WHERE iddocsarchives=$iddocsarchives", $dbh);

  // create log
      $result16 = mysql_query("SELECT adminuid FROM tbladminlogin WHERE adminloginid=$loginid", $dbh);
      while($myrow16 = mysql_fetch_row($result16))
      { $adminuid16=$myrow16[0]; }
      $adminlogdetails = "$loginid:$adminloginuid - deleted an item in Docs Archives id:$iddocsarchives, title:$title10, desc:$description10, dept:$deptcd10, file:$filename10";
	...
	// redirect
    header("Location: docsarchive.php?loginid=$loginid&dpt=$deptcd10");
    exit;
