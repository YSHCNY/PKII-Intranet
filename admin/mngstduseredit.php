...
if ($found == 1) {
  ...
  if($accesslevel >= 4 && $accesslevel <= 5) {
    ...
    echo "</div>";
    echo "<form id='saveForm' action='mngstduseredit2.php?loginid=$loginid&stdlid=$loginid0' method='post'>";
    ...
    if ((strpos($remarks_login11, "disabled") !== false) || ($attempt15 >= $usrpwretries) || ($disabled16b == 1)) {
        $disabledsel = "selected";
    } else {
        $enabledsel = "selected";
    }
    ...
    echo "<button type='button' class='btn mx-2 mainbtnclr text-white' data-toggle='modal' data-target='#confirmModal'>Save</button>";
    echo "</div>";
