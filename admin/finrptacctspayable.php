...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"finrptacctspayable.php?loginid=$loginid\" method=\"post\" name=\"finrptacctspayable\"><div class='form-inline'><div class='form-group'>";
    echo "<td colspan=\"2\">";
    ...
    echo "</tr>";
		...
		echo "<tr><th colspan=\"2\" align=\"left\">Accounts Payable Book <a href=\"#\" id=\"exportToExcel\"><img src=\"./images/sheet.gif\"></a></th></tr>";
    ...
    echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
		echo "<tr><th colspan=\"4\"></th><th colspan=\"2\">Accounts Payable</th><th colspan=\"3\">Advances to Client</th><th></th><th colspan=\"2\">Advances for Liquidation</th><th colspan=\"3\">Gen. & Adm. Expense</th><th colspan=\"3\">Direct Cost</th><th colspan=\"3\">Other Accounts</th></tr>";
    echo "<tr><th>Date</th><th>Ref</th><th>Payee</th><th>Particulars</th><th>Debit</th><th>Credit</th><th>Project</th><th>Debit</th><th>Credit</th><th>InputVAT</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th></tr>";

		$result11=""; $found11=0;
		$result11 = mysql_query("SELECT tblfinacctspayable.acctspayablenumber, tblfinacctspayable.payee, tblfinacctspayable.date, tblfinacctspayable.glcode, tblfinacctspayable.glrefver, tblfinacctspayable.projcode, tblfinacctspayable.particulars, tblfinacctspayable.debitamt, tblfinacctspayable.creditamt, tblfinacctspayable.company_id, tblfinacctspayable.contact_id, tblfinacctspayabletot.explanation FROM tblfinacctspayable LEFT JOIN tblfinacctspayabletot ON tblfinacctspayable.acctspayablenumber=tblfinacctspayabletot.acctspayablenumber WHERE (tblfinacctspayable.date >= \"$mmcutstart\" AND tblfinacctspayable.date <= \"$mmcutend\") GROUP BY tblfinacctspayable.acctspayablenumber ORDER BY tblfinacctspayable.date ASC, tblfinacctspayable.acctspayablenumber ASC", $dbh);
		if($result11 != "") {
			while($myrow11 = mysql_fetch_row($result11)) {
			$found11 = 1;
			$disbursementnumber11 = $myrow11[0];
			$payee11 = $myrow11[1];
			$payee11 = str_replace("'", "", $payee11);
			$date11 = $myrow11[2];
			$glcode11 = $myrow11[3];
		...
		// }
			...
			echo "<td align=\"right\">";
			$result12=""; $found12=0;
			$result12 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode=\"20.10.100\") AND glrefver=2", $dbh);
			if($result12 != "") {
			...
			$result12b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode=\"20.10.100\") AND glrefver=2", $dbh);
			if($result12b != "") {
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result15=""; $found15=0;
			$result15 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.404\" AND glrefver=2", $dbh);
			if($result15 != "") {
    ...
    if($proj_sname19 == '')
    {
      $proj_sname19 =  substr($proj_fname19, 0, 35);
			...
			echo "<td align=\"right\">";
			$result16=""; $found16=0;
			$result16 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.467\" AND glrefver=2", $dbh);
			if($result16 != "") {
			...
			echo "<td align=\"right\">";
			$result17=""; $found17=0;
			$result17 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.401\" AND glrefver=2", $dbh);
			if($result17 != "") {
			...
			$result17b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.401\" AND glrefver=2", $dbh);
			if($result17b != "") {
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result18=""; $found18=0;
			$result18 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"70.00.000\" AND glcode<=\"70.99.999\") AND glrefver=2", $dbh);
			if($result18 != "") {
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result19=""; $found19=0;
			$result19 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"60.00.000\" AND glcode<=\"60.99.999\") AND glrefver=2", $dbh);
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result20=""; $found20=0;
			$result20 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfinacctspayable WHERE acctspayablenumber=\"$disbursementnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode>=\"10.00.000\" AND glcode<=\"10.10.119\") OR (glcode>=\"10.10.120\" AND glcode<=\"10.10.121\") OR (glcode>=\"10.10.121.C\" AND glcode<=\"10.10.124.B\") OR (glcode>=\"10.10.124.D\" AND glcode<=\"10.10.400\") OR (glcode>=\"10.10.402\" AND glcode<=\"10.10.403\") OR (glcode>=\"10.10.405\" AND glcode<\"10.10.467\") OR (glcode>=\"10.20.000\" AND glcode<=\"10.40.999\") OR (glcode>=\"20.10.200\" AND glcode<=\"50.99.999\") OR (glcode>=\"71.00.000\" AND glcode<=\"99.99.999\")) AND glrefver=2 ORDER BY glcode ASC", $dbh);
			if($result20 != "") {
    ...
    echo "<tr><th colspan=\"4\" align=\"right\">Total</th><th class='text-right'>".number_format($debitamt12tot, 2)."</th><th class='text-right'>".number_format($creditamt12tot, 2).
"</th>";
		/* echo "<th class='text-right'></th><th class='text-right'>".number_format($debitamt14tot, 2)."</th><th class='text-right'>".number_format($creditamt14tot, 2).
"</th>"; */
		echo "<th class='text-right'></th><th class='text-right'>".number_format($debitamt15tot, 2)."</th><th class='text-right'>".number_format($creditamt15tot, 2).
"</th>";
		echo "<th class='text-right'>".number_format($amt16tot, 2)."</th>";
		// echo "<th align=\"right\">$amt16tot|$amt16totfin</th>";
		echo "<th class='text-right'>".number_format($debitamt17tot, 2)."</th><th class='text-right'>".number_format($creditamt17tot, 2).
"</th>";
		echo "<th class='text-right'></th><th class='text-right'>".number_format($debitamt18tot, 2)."</th><th class='text-right'>".number_format($creditamt18tot, 2).
"</th>";
		echo "<th class='text-right'></th><th class='text-right'>".number_format($debitamt19tot, 2)."</th><th class='text-right'>".number_format($creditamt19tot, 2).
"</th>";
		echo "<th class='text-right'></th><th class='text-right'>".number_format($debitamt20tot, 2)."</th><th class='text-right'>".number_format($creditamt20tot, 2).
"</th>";
		echo "</tr>";

		echo "<tr><th colspan=\"4\" align=\"right\">Grand-total</th>";
		echo "<th class='text-right'>".number_format($debitgrandtot, 2)."</th>";
		echo "<th class='text-right'>".number_format($creditgrandtot, 2)."</th>";
		echo "<td colspan=\"15\"></td></tr>";
