...
// start contents here
    ...
    echo "<tr><th colspan=\"2\">Project's Journal Summary</th></tr>";
    echo "<tr>";
    echo "<form action=\"finrptprojjournal.php?loginid=$loginid\" method=\"post\" name=\"form1\" id=\"form1\">";
    ...
    echo "<option>Year-Month</option>";
		...
		$result11=""; $found11=0;
		$result11 = mysql_query("SELECT DISTINCT DATE_FORMAT(date, '%Y %M') as yyyymonth, projcode FROM tblfinjournal WHERE projcode=\"$projcodesel\" ORDER BY date DESC", $dbh);
    while($myrow11 = mysql_fetch_row($result11))
    {
      $found11 = 1;
      $yyyymonth = $myrow11[0];
    ...
    echo "</select>";
		...
		$result14=""; $found14=0;
		$result14 = mysql_query("SELECT DISTINCT DATE_FORMAT(date, '%Y') as yyyy, projcode FROM tblfinjournal WHERE projcode=\"$projcodesel\" ORDER BY date DESC", $dbh);
		if($result14 != "") {
		...
		$result12=""; $found12=0;
		$result12 = mysql_query("SELECT DISTINCT DATE_FORMAT(date, '%Y') as yyyy, projcode FROM tblfinjournal WHERE projcode=\"$projcodesel\" ORDER BY date DESC", $dbh);
		if($result12 != "") {
    ...
    echo "<tr><th>count</th><th>jvnum</th><th>date</th><th>particulars</th><th>dr</th><th>cr</th></tr>";

  if($projcodesel != "") {
		if($schedule != "all") {
    $result12 = mysql_query("SELECT journalnumber, date, projcode, particulars, debitamt, creditamt FROM tblfinjournal WHERE projcode=\"$projcodesel\" AND date>=\"$dtcutstart\" AND date<=\"$dtcutend\" ORDER BY date DESC", $dbh);
		} else {
    $result12 = mysql_query("SELECT journalnumber, date, projcode, particulars, debitamt, creditamt FROM tblfinjournal WHERE projcode=\"$projcodesel\" ORDER BY date DESC", $dbh);
		}
    if($result12 != "") {
    while($myrow12 = mysql_fetch_row($result12)) {
      $found12 = 1;
      $journalnumber12 = $myrow12[0];
      $date12 = $myrow12[1];
      ...
      echo "<tr><td>$count12</td><td><a href=\"finvouchjvview.php?loginid=$loginid&jvn=$journalnumber12\" target=\"_blank\">$journalnumber12</a></td><td><a href=\"finvouchjvview.php?loginid=$loginid&jvn=$journalnumber12\" target=\"_blank\">$date12</a></td><td>$particulars12</td><td align=\"right\">".number_format($debitamt12, 2)."</td><td align=\"right\">".number_format($creditamt12, 2)."</td></tr>";
    ...
    echo "<tr><th colspan=\"4\" align=\"right\">Total</th><th align=\"right\">".number_format($debittot, 2)."</th><th align=\"right\">".number_format($credittot, 2).
"</th></tr>";
