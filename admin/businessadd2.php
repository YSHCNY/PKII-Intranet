...
if ($found == 1)
{
  ...
  if($accesslevel >= 4 && $accesslevel <= 5)
  {
    ...
    $result11 = 0;
    $result11 = mysql_query("SELECT supplierid, companyid, company, branch FROM tblcompany WHERE supplierid=\"$supplierid\" AND company=\"$company\"", $dbh);
    if ($result11 != '')
    {
    ...
    if($found11 == 1)
    {
      echo "<tr><td>Warning</td><td><font color=\"red\">Sorry, company name and/or branch already exists. Please try again.</font></td></tr>";
    }
    else
    {
			$res12query="INSERT INTO tblcompany SET company=\"$company\", tin_number=\"$tin_number\", branch=\"$branch\", ofc_address1=\"$ofc_address1\", ofc_address2=\"$ofc_address2\", ofc_city=\"$ofc_city\", ofc_province=\"$ofc_province\", ofc_zipcode=\"$ofc_zipcode\", ofc_country=\"$ofc_country\", ofc_num1_cc=\"$ofc_num1_cc\", ofc_num1_ac=\"$ofc_num1_ac\", ofc_num1=\"$ofc_num1\", ofc_num1_ext=\"$ofc_num1_ext\", ofc_num2_cc=\"$ofc_num2_cc\", ofc_num2_ac=\"$ofc_num2_ac\", ofc_num2=\"$ofc_num2\", ofc_num2_ext=\"$ofc_num2_ext\", ofc_num3_cc=\"$ofc_num3_cc\", ofc_num3_ac=\"$ofc_num3_ac\", ofc_num3=\"$ofc_num3\", ofc_num3_ext=\"$ofc_num3_ext\", ofc_fax_cc=\"$ofc_fax_cc\", ofc_fax_ac=\"$ofc_fax_ac\", ofc_fax=\"$ofc_fax\", ofc_fax2_cc=\"$ofc_fax2_cc\", ofc_fax2_ac=\"$ofc_fax2_ac\", ofc_fax2=\"$ofc_fax2\", ofc_mobile_cc=\"$ofc_mobile_cc\", ofc_mobile_ac=\"$ofc_mobile_ac\", ofc_mobile=\"$ofc_mobile\", ofc_email=\"$ofc_email\", ofc_url=\"$ofc_url\", products=\"$products\", services=\"$services\", remarks_company=\"$remarks_company\", company_type=\"$company_type\", supplierid=\"$supplierid\", contactid=\"$contactid\", proj_code=\"$proj_code\", employeeid=\"$employeeid0\"";
      ...
      echo "type=$company_type, supplierid:$supplierid, contactid:$contactid, proj_code:$proj_code, employeeid:$employeeid0<br>";
			// echo "$res12query<br>";
      echo "</td></tr>";
      ...
      $adminlogdetails = "$loginid:$adminloginuid - add new company record:$company $branch $company_type";
