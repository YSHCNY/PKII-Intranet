...
if ($found == 1)
{
  ...
  $result2 = mysql_query("UPDATE tblempdependent SET dependentbirthdate = '$dependentbirthdate', dependentlast='$dependentlast', dependentfirst='$dependentfirst',
...
//   echo "<p><a href=personnelempdependentedit.php?loginid=$loginid&eid=$employeeid&did=$empdependentid>Back to Edit Dependent</a></p>";
  ...
  $message = "Dependent Details Updated Sucessfully!";
  $_SESSION['success_message'] = $message;
?>
  <script>
          const employeeid = encodeURIComponent("<?php echo $employeeid; ?>");
          const loginid = encodeURIComponent("<?php echo $loginid; ?>");
          const insuranceempid = encodeURIComponent("<?php echo $empdependentid; ?>");

          window.location.href = `personnelempdependentedit.php?loginid=${loginid}&eid=${employeeid}&did=${insuranceempid}`;
     </script>
        <?php
     include ("footer.php");
}
