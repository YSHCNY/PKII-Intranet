...
include '../includes/dbh.php';
	...
	$res11query="SELECT tblemployee.emp_birthdate, tblcontact.employeeid, tblcontact.name_last, tblcontact.name_first, tblcontact.name_middle, tblcontact.picfn FROM tblemployee LEFT JOIN tblcontact ON tblemployee.employeeid=tblcontact.employeeid WHERE (DATE(CONCAT(YEAR(CURDATE()), RIGHT(tblemployee.emp_birthdate, 6))) BETWEEN NOW() - INTERVAL 2 DAY AND NOW() + INTERVAL 15 DAY) AND tblemployee.emp_record='active' AND tblemployee.employee_type='employee' ORDER BY DATE_FORMAT(tblemployee.emp_birthdate, '%m-%d');";
