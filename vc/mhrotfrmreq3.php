...
if ($apprctr==1){
	// echo "approved <br> $overtimedate, $apprctr, $idhrtaotreq, $empid";
	$resquery="UPDATE tblhrtaotreq SET statusta = 1 WHERE idhrtaotreq = $idhrtaotreq";
	$result=$dbh->query($resquery);
	$sqlapprove = $dbh->prepare("UPDATE tblhrtaemptimelog SET otafteroutsw = ? WHERE logdate = '$overtimedate' AND employeeid = $empid");
	$sqlapprove->bind_param("i", $approveit);
	$sqlapprove->execute();


	$messageAppr = "Success! Overtime Approved!";
...
echo "<script>
	...
	$resquery="UPDATE tblhrtaotreq SET statusta = 2 WHERE idhrtaotreq = $idhrtaotreq";
	$result=$dbh->query($resquery);

	$sqlapprove = $dbh->prepare("UPDATE tblhrtaemptimelog SET otafteroutsw = ? WHERE logdate = '$overtimedate' AND employeeid = $empid");
	$sqlapprove->bind_param("i", $disapproveit);
	$sqlapprove->execute();


	
	$messageDenied = "Overtime Denied!";
