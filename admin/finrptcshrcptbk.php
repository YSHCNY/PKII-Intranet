...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"finrptcshrcptbk.php?loginid=$loginid\" method=\"post\"><div class='form-inline'><div class='form-group'>";
    echo "<td colspan=\"2\">";
    ...
    echo "</tr>";
		...
		echo "<tr><th colspan=\"2\" align=\"left\">Cash Receipts Book <a href=\"#\" id=\"exportToExcel\"><img src=\"./images/sheet.gif\"></a></th></tr>";
    ...
    echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
		echo "<tr><th colspan=\"4\"></th><th colspan=\"2\">Cash in Bank-BPI</th><th colspan=\"2\">Cash in Bank-Mizuho USD</th><th colspan=\"2\">AR Trade</th><th></th><th colspan=\"2\">Advances for Liquidation</th><th colspan=\"3\">Gen. & Adm. Expense</th><th colspan=\"3\">Direct Cost</th><th colspan=\"3\">Other Accounts</th></tr>";
    echo "<tr><th>Date</th><th>Ref</th><th>Payee</th><th>Particulars</th><th>Debit</th><th>Credit</th><th>Debit</th><th>Credit</th><th>Debit</th><th>Credit</th><th>InputVAT</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th><th>AcctName</th><th>Debit</th><th>Credit</th></tr>";

		$result11=""; $found11=0;
		$result11 = mysql_query("SELECT cashreceiptnumber, payor, date, glcode, glrefver, glnamedetails, projcode, particulars, debitamt, creditamt, explanation, companyid, contactid FROM tblfincashreceipt WHERE (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") GROUP BY cashreceiptnumber ORDER BY date ASC, cashreceiptnumber ASC", $dbh);
		if($result11 != "") {
			while($myrow11 = mysql_fetch_row($result11)) {
			$found11 = 1;
			...
			$explanation11 = $myrow11[10];
			$companyid11 = $myrow11[11];
			$contactid11 = $myrow11[12];
			$explanationfin = str_replace("'", "", $explanation11);

			echo "<tr>";
			echo "<td>".date("d-M-y", strtotime($date11))."</td><td>$cashreceiptnumber11</td><td>";
			// echo "$payor11";
		...
		if((($contactid11!="") || ($contactid11!=0)) && (($companyid11=="") || ($companyid11==0))) {
			...
			echo "".str_replace("'", "", $contactname15bfin)."";
		}
		if((($companyid11=="") && ($contactid11=="")) || (($companyid11==0) && ($contactid11==0))) {
			echo "".str_replace("'", "", $payor11)."";
		}

			echo "</td><td>$explanationfin</td>";

			// generate CIB-BPI debit side
			echo "<td align=\"right\">";
			$result12=""; $found12=0;
			$result12 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"10.10.121.A\" AND glcode<=\"10.10.121.B\") AND glrefver=2", $dbh);
			if($result12 != "") {
			...
			// generate CIB-BPI credit side
			echo "<td align=\"right\">";
			$result12b=""; $found12b=0;
			$result12b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"10.10.121.A\" AND glcode<=\"10.10.121.B\") AND glrefver=2", $dbh);
			if($result12b != "") {
...
/*
			// generate CIB-BOT Yen debit side
			echo "<td align=\"right\">";
			$result14=""; $found14=0;
			$result14 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.122.D\" AND glrefver=2", $dbh);
			if($result14 != "") {
			...
			// generate CIB-BOT Yen credit side
			echo "<td align=\"right\">";
			$result14b=""; $found14b=0;
			$result14b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.122.D\" AND glrefver=2", $dbh);
			if($result14b != "") {
...
*/
			...
			// generate CIB-Mizuho USD debit side
			echo "<td align=\"right\">";
			$result14=""; $found14=0;
			$result14 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.124.C\" AND glrefver=2", $dbh);
			if($result14 != "") {
				while($myrow14 = mysql_fetch_row($result14)) {
				$found14 = 1;
				$glcode14 = $myrow14[0];
			...
			// generate CIB-Mizuho USD credit side
			echo "<td align=\"right\">";
			$result14b=""; $found14b=0;
			$result14b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.124.C\" AND glrefver=2", $dbh);
			if($result14b != "") {
				while($myrow14b = mysql_fetch_row($result14b)) {
				$found14b = 1;
				$glcode14b = $myrow14b[0];
			...
			// generate AR Trade debit side
			echo "<td align=\"right\">";
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result15=""; $found15=0;
			// $result15 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"10.10.300\" AND glcode<=\"10.10.305.1\") AND glrefver=2", $dbh);
			$result15 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode=\"10.10.300\") OR (glcode>=\"10.10.301.1\" AND glcode<=\"10.10.305.1\")) AND glrefver=2", $dbh);
			if($result15 != "") {
				...
				echo "<tr>";
				echo "<td align=\"right\">".number_format($debitamt15, 2)."</td>";
				echo "</tr>";
				}
			}
			echo "</table>";
			...
			// generate AR Trade credit side
			echo "<td align=\"right\">";
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result15b=""; $found15b=0;
			// $result15b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"10.10.300\" AND glcode<=\"10.10.305.1\") AND glrefver=2", $dbh);
			$result15b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode=\"10.10.300\") OR (glcode>=\"10.10.301.1\" AND glcode<=\"10.10.305.1\")) AND glrefver=2", $dbh);
			if($result15b != "") {
				while($myrow15b = mysql_fetch_row($result15b)) {
				$found15b = 1;
				...
				echo "<tr>";
				echo "<td align=\"right\">".number_format($creditamt15b, 2)."</td>";
				echo "</tr>";
				}
			}
			echo "</table>";
			...
			echo "<td align=\"right\">";
			$result16=""; $found16=0;
			$result16 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.467\" AND glrefver=2", $dbh);
			if($result16 != "") {
			...
			// generate Adv for Liquidation debit side
			echo "<td align=\"right\">";
			$result17=""; $found17=0;
			$result17 = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.401\" AND glrefver=2", $dbh);
			if($result17 != "") {
			...
			// generate Adv for Liquidation credit side
			echo "<td align=\"right\">";
			$result17b=""; $found17b=0;
			$result17b = mysql_query("SELECT glcode, projcode, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND glcode=\"10.10.401\" AND glrefver=2", $dbh);
			if($result17b != "") {
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result18=""; $found18=0;
			$result18 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"70.00.000\" AND glcode<=\"70.99.999\") AND glrefver=2", $dbh);
			if($result18 != "") {
			...
			// generate Direct Costs
			echo "<td colspan=\"3\" align=\"right\">";
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result19=""; $found19=0;
			$result19 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND (glcode>=\"60.00.000\" AND glcode<=\"60.99.999\") AND glrefver=2", $dbh);
			if($result19 != "") {
			...
			echo "<table width=\"100%\" class=\"fin2\" border=\"1\" spacing=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
			$result20=""; $found20=0;
			// $result20 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode>=\"10.00.000\" AND glcode<=\"10.10.119\") OR (glcode>=\"10.10.121.C\" AND glcode<=\"10.10.122.C\") OR (glcode>=\"10.10.123.A\" AND glcode<=\"10.10.299\") OR (glcode>=\"10.10.350\" AND glcode<=\"10.10.399\") OR (glcode>=\"10.10.402\" AND glcode<=\"10.10.466\") OR (glcode>=\"10.20.000\" AND glcode<=\"50.99.999\") OR (glcode>=\"71.00.000\" AND glcode<=\"99.99.999\")) AND glrefver=2 ORDER BY glcode ASC", $dbh);
// 10.10.122.D bot yen
// 10.10.124.C mizuho usd
			$result20 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode>=\"10.00.000\" AND glcode<=\"10.10.119\") OR (glcode>=\"10.10.120\" AND glcode<=\"10.10.121\") OR (glcode>=\"10.10.121.C\" AND glcode<=\"10.10.124.B\") OR (glcode>=\"10.10.124.D\" AND glcode<=\"10.10.299\") OR (glcode=\"10.10.300.A\") OR (glcode>=\"10.10.350\" AND glcode<=\"10.10.399\") OR (glcode=\"10.10.400\") OR (glcode>=\"10.10.402\" AND glcode<\"10.10.467\") OR (glcode>=\"10.20.000\" AND glcode<=\"50.99.999\") OR (glcode>=\"71.00.000\" AND glcode<=\"99.99.999\")) AND glrefver=2 ORDER BY glcode ASC", $dbh);
			if($result20 != "") {
...
"</th>";
		echo "<th align=\"right\">".number_format($debitamt15tot, 2)."</th><th align=\"right\">".number_format($creditamt15tot, 2).
"</th>";
...
"</th>";
		...
		// display grand-total
		echo "<tr><th colspan=\"4\" align=\"right\">Grand-total</th><th align=\"right\">".number_format($debitgrandtot, 2)."</th><th align=\"right\">".number_format($creditgrandtot, 2)."</th><td colspan=\"16\"></td></tr>";
