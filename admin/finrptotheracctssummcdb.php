...
// start contents here
    ...
    echo "<tr>";
    echo "<form action=\"finrptotheracctssummcdb.php?loginid=$loginid\" method=\"post\"><div class='form-inline'><div class='form-group'>";
    echo "<td colspan=\"2\">";
    ...
    echo "</tr>";
		...
		$result11=""; $found11=0;
		$result11 = mysql_query("SELECT tblfindisbursement.disbursementnumber, tblfindisbursement.payee, tblfindisbursement.date, tblfindisbursement.glcode, tblfindisbursement.glrefver, tblfindisbursement.projcode, tblfindisbursement.particulars, tblfindisbursement.debitamt, tblfindisbursement.creditamt FROM tblfindisbursement WHERE (tblfindisbursement.date >= \"$mmcutstart\" AND tblfindisbursement.date <= \"$mmcutend\") AND ((tblfindisbursement.glcode>=\"10.00.000\" AND tblfindisbursement.glcode<=\"10.10.119\") OR (tblfindisbursement.glcode>=\"10.10.120\" AND tblfindisbursement.glcode<=\"10.10.121\") OR (tblfindisbursement.glcode>=\"10.10.121.C\" AND tblfindisbursement.glcode<=\"10.10.124.B\") OR (tblfindisbursement.glcode>=\"10.10.124.D\" AND tblfindisbursement.glcode<=\"10.10.400\") OR (tblfindisbursement.glcode>=\"10.10.402\" AND tblfindisbursement.glcode<=\"10.10.403\") OR (tblfindisbursement.glcode>=\"10.10.405\" AND tblfindisbursement.glcode<\"10.10.467\") OR (tblfindisbursement.glcode>=\"10.20.000\" AND tblfindisbursement.glcode<=\"50.99.999\") OR (tblfindisbursement.glcode>=\"71.00.000\" AND tblfindisbursement.glcode<=\"99.99.999\")) AND tblfindisbursement.glrefver=2 GROUP BY tblfindisbursement.glcode ORDER BY tblfindisbursement.glcode ASC, tblfindisbursement.disbursementnumber ASC", $dbh);
		if($result11 != "") {
			while($myrow11 = mysql_fetch_row($result11)) {
			$found11 = 1;
			...
			$result20=""; $found20=0;
			$result20 = mysql_query("SELECT glcode, projcode, particulars, debitamt, creditamt FROM tblfindisbursement WHERE glcode=\"$glcode11\" AND (date>=\"$mmcutstart\" AND date<=\"$mmcutend\") AND ((glcode>=\"10.00.000\" AND glcode<=\"10.10.119\") OR (glcode>=\"10.10.120\" AND glcode<=\"10.10.121\") OR (glcode>=\"10.10.121.C\" AND glcode<=\"10.10.124.B\") OR (glcode>=\"10.10.124.D\" AND glcode<=\"10.10.400\") OR (glcode>=\"10.10.402\" AND glcode<=\"10.10.403\") OR (glcode>=\"10.10.405\" AND glcode<\"10.10.467\") OR (glcode>=\"10.20.000\" AND glcode<=\"50.99.999\") OR (glcode>=\"71.00.000\" AND glcode<=\"99.99.999\")) AND glrefver=2 ORDER BY glcode ASC", $dbh);
			if($result20 != "") {
