...
<?php
	//  echo "<a  class=\"btn mainbtnclr text-white mb-5\" href=\"hrtimeatt.php?loginid=$loginid\">Back</a>";
	include 'timeattmenu.php';

  if($accesslevel >= 4) {

		echo "<form action=\"hrtimeattindivinfo.php?loginid=$loginid\" method=\"post\" name=\"modhrtaindivinfo\" class = 'shadow px-5 py-3'>";
		echo "<div class = 'my-5'><h5 class = 'pb-0 mb-0 fw-bold'>Manage Employee Information</h5><p>Update or Change employee status for TAL module </p></div>";
		echo "<div class = 'row'>";

		// pay group name dropdown
    echo "<div class = 'col'><select name=\"idpaygroup\" class = 'GlobalSelectWx w-100' onchange=\"this.form.submit()\">";
	...
	//
	if($employeeid != "") {
	echo "<div class = 'shadow p-4'>";
	echo "<table width=\"100%\" class = 'table table-bordered table-striped  table-hover table-lg'>";
	...
	*/
	...
	// row0
	echo "<tr><th colspan=\"6\">Details for employee ID: $employeeid</th></tr>";
	/*
	...
	*/
	...
	$res12query = "SELECT * FROM tblemployee WHERE employeeid=\"$employeeid\"";
	...
	if($result12->num_rows>0) {
		...
		$vl = $myrow12['vacation'];
		$sl = $myrow12['sick'];
		$pl = $myrow12['paternity'];
		$mc = $myrow12['maternityc'];
		$mn = $myrow12['maternityn'];
		$spl = $myrow12['special'];
		$asl = $myrow12['aspl'];
	...
	// if($found17==1) {
	// $vlbal17=$bal17;
	echo "<input name=\"vlbal\" size=\"3\" value=\"$vl\">";
	// } else {
	...
	// if($found17==1) {
	// $slbal17=$bal17;
	echo "<input name=\"slbal\" size=\"3\" value=\"$sl\">";
	// } else {
	// // query tblhrtaleavectg for quota
	...
	// if($found17==1) {
	// $paterbal17=$bal17;
	echo "<input name=\"paterbal\" size=\"3\" value=\"$pl\">";
	// } else {
	// // query tblhrtaleavectg for quota
	...
	// if($found17==1) {
	// $maternbal17=$bal17;
	echo "<input name=\"maternbal\" size=\"3\" value=\"$mn\">";
	// } else {
	// // query tblhrtaleavectg for quota
	...
	// if($found17==1) {
	// $matercbal17=$bal17;
	echo "<input name=\"matercbal\" size=\"3\" value=\"$mc\">";
	// } else {
	// // query tblhrtaleavectg for quota
	...
	// if($found17==1) {
	// $splbal17=$bal17;
	echo "<input name=\"splbal\" size=\"3\" value=\"$spl\">";
	// } else {
	// // query tblhrtaleavectg for quota
  ...
  echo "<td align=\"right\">Employment status:</td><th align=\"left\">$emp_status12</th>";
	...
	// 20240731 to add radio for flexi-Time
	if($flexitime14==1) {
		$flexitimesel="checked=\"checked\""; $nonflexitimesel="";
	} else if($flexitime14==0) {
	    $flexitimesel=""; $nonflexitimesel="checked=\"checked\"";
	} else {
	    $flexitimesel=""; $nonflexitimesel="checked=\"checked\"";	
	} //if-else
	echo "<input type='radio' name='flexitime' value='1' $flexitimesel>&nbsp;Flexi-time (7-10AM to 4-7PM)";
	echo "<br>";
	echo "<input type='radio' name='flexitime' value='0' $nonflexitimesel>&nbsp;";
...
echo "<td align=\"right\">Rest day</td><td align=\"left\" colspan=\"3\">";
	...
	*/
	...
	echo "<tr><td colspan=\"6\" align=\"center\"><button type=\"submit\" class=\"btn text-white bg-success\">Update</button></td></tr>";
	echo "</form>";
	echo "</table>";
	echo "</td></tr>";
	}
