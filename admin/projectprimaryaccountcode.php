...
include("tjfunctions/tjclasses.php");
	 ...
	 $query = "Select * from tblfinprojinprimary where status ='1'";
...
$sql = "SELECT * from tblfinprojinprimary Where 1=1 AND status ='1'";
...
while( $row=$res->fetch_assoc() ) {  // preparing an array
	...
	$nestedData[] = date('F d, Y', strtotime($row["datecreated"]));
	$nestedData[] = $btn;
