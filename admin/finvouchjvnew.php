...
if($jvnumber0 != '') {
    ...
    $res16query = "SELECT journaltotid, journalnumber, date, explanation, debittot, credittot, status, filepath, filename FROM tblfinjournaltot WHERE journalnumber=\"$jvnumber0\"";
...
//    echo "<form action=\"finvouchjvadd.php?loginid=$loginid\" method=\"post\" name=\"myForm\">";
    echo "<tr><td>Date:&nbsp;<b><input name=\"jvdate\" value=\"$date15\" size=\"12\" readonly></b><a href=\"finvouchjvchgdate.php?loginid=$loginid&jvn=$jvnumber0&jvdt=$date15\" class='btn btn-primary' role='button'><font size=\"1\"><i>Change</i></font></a></td><td>J.V. No.:&nbsp;<b><input name=\"jvnumber\" value=\"$journalnumber15\" size=\"12\" readonly></b><a href=\"finvouchjvchgnum.php?loginid=$loginid&jvn=$jvnumber0&jvdt=$date15\" class='btn btn-primary' role='button'><font size=\"1\"><i>Change</i></font></a></td></tr>";
    ...
    echo "<textarea rows=\"4\" cols=\"60\" name=\"explanation\">$explanationfin</textarea><font size=\"1\"><i><a href=\"finvouchjvchgexpla.php?loginid=$loginid&jvn=$journalnumber15&jvdt=$date15\" class='btn btn-primary' role='button'>Change</a></i></font></td></tr>";

    //
	// 20250505 file uploader
	echo "<tr><td colspan='2'>File attachment&nbsp;";
    if($filename16 != "") {
    echo "<a href=\"$filepath16/$filename16\" target=\"_blank\">$filename16</a>&nbsp;&nbsp;&nbsp;<i><a href=\"finvouchjvdelfile.php?loginid=$loginid&jvtid=$journaltotid16&jvn=$jvnumber0\">Remove</a></i><br>";    
	} else {
	echo "<form enctype=\"multipart/form-data\" action=\"finvouchjvfileupload.php?loginid=$loginid&jvtid=$journaltotid16&jvn=$jvnumber0\" method=\"POST\" name=\"finvouchjvfileupload\">";
	echo "<input type=\"hidden\" name=\"journaltotid\" value=\"$journaltotid16\">";
	echo "<input type=\"hidden\" name=\"journalnumber\" value=\"$jvnumber0\">";
	...
	echo "<button type='submit' class='btn btn-success' name='btnJVfileupload' value=1>Save uploaded file</button>";		
	echo "</form>";
	} // if($filename16 != "")
	echo "</td></tr>";
	
	// relocate form here
    echo "<form action=\"finvouchjvadd.php?loginid=$loginid\" method=\"post\" name=\"myForm\">"; // fr line:98
	echo "<input type=\"hidden\" name=\"jvdate\" value=\"$date15\">";
	echo "<input type=\"hidden\" name=\"jvnumber\" value=\"$journalnumber15\">";
