...
// combine glname, glnamedetails, projname, particulars to particulars variable
  ...
  $result12 = mysql_query("UPDATE tblfincashreceipt SET glcode=\"$glcode\", glrefver=\"$version17\", glnamedetails=\"$glnamedetails\", projcode=\"$projcode\", particulars=\"$allparticulars\", debitamt=\"$debitamt\", creditamt=\"$creditamt\" WHERE cashreceiptid=$cashreceiptid0 AND cashreceiptnumber=\"$cashreceiptnumber0\"", $dbh);


// update debittot and credittot
  $result11 = mysql_query("SELECT date, debitamt, creditamt FROM tblfincashreceipt WHERE cashreceiptnumber=\"$cashreceiptnumber0\"", $dbh);
  ...
  if($creditamt == "") { $creditamt = 0; }
	...
	$result16=""; $found16=0;
  $result16 = mysql_query("SELECT cashreceipttotid, explanation FROM tblfincashreceipttot WHERE cashreceiptnumber=\"$cashreceiptnumber0\"", $dbh);
  ...
  if($found16 == 1)
  {
    $result14 = mysql_query("UPDATE tblfincashreceipttot SET date=\"$datenow\", debittot=$debittot11, credittot=$credittot11, status=\"$status\" WHERE cashreceipttotid=$cashreceipttotid16 AND cashreceiptnumber=\"$cashreceiptnumber0\"", $dbh);
  }
  else
  {
    $result15 = mysql_query("INSERT INTO tblfincashreceipttot SET cashreceiptnumber=\"$cashreceiptnumber0\", date=\"$date11\", explanation=\"$explanationfin\", debittot=$debittot11, credittot=$credittot11, status=\"$status\"", $dbh);
  }
...
// create log
    ...
    while($myrow16 = mysql_fetch_row($result16))
    { $adminuid=$myrow16[0]; }
    $adminlogdetails = "$loginid:$adminloginuid - Modified Cash Receipt Voucher item with CRV:$cashreceiptnumber0 - details:$allparticulars, acctcode:$glcode, debit:$debitamt, credit:$creditamt";
    $result17 = mysql_query("INSERT INTO tbladminlogs SET adminloginid=$loginid, timestamp=\"$now\", adminuid=\"$adminuid\", adminlogdetails=\"$adminlogdetails\"", $dbh);

}

  header("Location: finvouchcrvnew.php?loginid=$loginid&crvn=$cashreceiptnumber0");
  exit;
